...
function DropdownMenuContent({
    ...
    return (
        <DropdownMenuContentPrimitive
            ...
            className={cn(
                'bg-sidebar text-popover-foreground z-50 max-h-(--radix-dropdown-menu-content-available-height) min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded border p-1 shadow-md outline-none',
                className,
            )}
            {...props}
        >
            <DropdownMenuHighlightPrimitive  mode="parent" className="absolute inset-0 bg-accent z-0 rounded">
                {children}
...
function DropdownMenuItem({
    ...
    // Radix's shared "Highlight" animation tracks which item is
    // hovered via a stable id/data-value read off the DOM node (see
    // DropdownMenuItem in the primitive file). Without one, the
    // highlighted-item tracking silently fails, causing the moving
    // highlight background to lag/snap incorrectly -- visible as a
    // double-flicker when hovering between items. useId() guarantees
    // a stable identifier per item instance without requiring every
    // call site to manually pass one.
    ...
    return (
        ...
        >
            <DropdownMenuItemPrimitive
                ...
                className={cn(
                    "focus:text-accent-foreground data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:text-destructive data-[variant=destructive]:*:[svg]:!text-destructive [&_svg:not([class*='text-'])]:text-muted-foreground relative flex items-center gap-2 rounded px-2 py-1.5 outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-[inset]:pl-8 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 cursor-pointer text-sm font-normal",
                    className,
                )}
                {...props}
            />
    ...
    return (
        <DropdownMenuHighlightItemPrimitive disabled={disabled}>
            <DropdownMenuCheckboxItemPrimitive
                disabled={disabled}
                className={cn(
                    "focus:text-accent-foreground relative flex cursor-default items-center gap-2 rounded py-1.5 pr-2 pl-8 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
                    className,
                )}
                checked={checked}
                {...props}
    ...
    return (
        <DropdownMenuHighlightItemPrimitive disabled={disabled}>
            <DropdownMenuSubTriggerPrimitive
                ...
                data-inset={inset}
                className={cn(
                    'focus:text-accent-foreground data-[state=open]:text-accent-foreground flex cursor-default items-center rounded px-2 py-1.5 text-sm outline-hidden select-none data-[inset]:pl-8',
...
function DropdownMenuSubContent({
    ...
    return (
        <DropdownMenuSubContentPrimitive
            className={cn(
                'bg-popover text-popover-foreground z-50 min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-hidden rounded border p-1 shadow-lg outline-none',
                className,
            )}
            {...props}
        />
