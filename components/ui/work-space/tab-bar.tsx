...
export default function TabBar({
    ...
    const textColor = 'text-ctr-ink';

    const { open: chatOpen, toggle: toggleChat } = useChatSidebar();

    return (
        <div className="flex h-[59px] flex-none items-center border-b border-ctr-ink-hair bg-ctr-paper px-2.5 py-2 font-ctr-sans">
            {/* LEFT */}
            <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto">
                    ...
                    return (
                        <div
                            ...
                            className={
                                ...
                                `rounded-[1px] border px-2.5 py-1.5 transition-colors duration-300 [transition-timing-function:var(--ease-ctr)] ` +
                                (active
                                    ? `border-ctr-ink-hair bg-ctr-paper-lift font-semibold shadow-[0_1px_0_rgba(43,58,74,0.14)] ${textColor}`
                                    : `border-transparent bg-ctr-paper ${textColor} hover:bg-ctr-paper-2`)
                            }
                        >
                            <FileIcon name={tab} />

                            <span className="truncate font-ctr-mono text-[12.5px]">{tab}</span>
                            ...
                            <span
                                ...
                                title="Close"
                                className={
                                    `flex h-[18px] w-[18px] flex-none items-center justify-center rounded-[1px] ` +
                                    `${textColor} opacity-60 transition-colors duration-300 [transition-timing-function:var(--ease-ctr)] ` +
                                    `hover:bg-ctr-paper-3 hover:opacity-100`
                                }
                            >
                ...
                <button
                    ...
                    title="New tab"
                    className={
                        `ml-1 flex h-7 w-7 flex-none cursor-pointer items-center justify-center rounded-[1px] ` +
                        `${textColor} transition-colors duration-300 [transition-timing-function:var(--ease-ctr)] hover:bg-ctr-paper-2`
                    }
                >
